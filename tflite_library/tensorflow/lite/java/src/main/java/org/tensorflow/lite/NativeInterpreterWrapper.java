...
final class NativeInterpreterWrapper implements AutoCloseable {
  ...
  private void applyDelegates(Interpreter.Options options) {
    ...
    try {
      ...
      if (options.useNNAPI != null && options.useNNAPI.booleanValue()) {
        NnApiDelegate optionalNnApiDelegate = null;
        
	if (options.m_accelerator != null || options.m_operation != null) {
	  optionalNnApiDelegate = new NnApiDelegate(options.m_accelerator, options.m_operation); // by LDY
	} else {
	  optionalNnApiDelegate = new NnApiDelegate();
        }
	if (m_devices == null || m_devices.length == 0) { // by LDY
          String[] devices = optionalNnApiDelegate.get_device_names(); // by LDY
          if ((devices != null) && (devices.length > 0)) {
            m_devices = new String[devices.length];
            for (int i = 0; i < devices.length; ++i) {
              m_devices[i] = new String(devices[i]);
            }
          } else {
            m_devices = new String[1];
            m_devices[0] = new String("");
          }
	}
	...
	m_NnApiDelegate = optionalNnApiDelegate; // by LDY
      }
  ...
  private String[] m_devices = null; // by LDY

  public String[] get_devices() { // by LDY
    String[] ret;
    ...
    ret = new String[m_devices.length];
    for (int i = 0; i < ret.length; ++i) {
      ret[i] = new String(m_devices[i]);
    }
    return ret;
  }

  private NnApiDelegate m_NnApiDelegate = null;
  public long get_hw_time() { // by LDY
    if (m_NnApiDelegate != null) {
      return m_NnApiDelegate.get_hw_exe_time();
    } else {
      return -1;
    }
  }

  public long get_driver_time() { // by LDY
    if (m_NnApiDelegate != null) {
      return m_NnApiDelegate.get_driver_exe_time();
    } else {
      return -1;
    }
  }
