...
TfLiteStatus NNAPIDelegateKernel::Prepare(TfLiteContext* context,
  ...
  if (nnapi_->android_sdk_version >= kMinSdkVersionForNNAPI12 ) {
    nnapi_->ANeuralNetworksBurst_create(nn_compilation_.get(), &m_burst); // by LDY
  }
  return kTfLiteOk;
}
...
TfLiteStatus NNAPIDelegateKernel::Invoke(TfLiteContext* context,
  ...
  if (nnapi_->android_sdk_version < kMinSdkVersionForNNAPI12) {
    ...
    int res = nnapi_->ANeuralNetworksExecution_setMeasureTiming(
      execution, true); // by LDY
    // Use synchronous execution for NNAPI 1.2+.
    RETURN_TFLITE_ERROR_IF_NN_ERROR(
        context, nnapi_->ANeuralNetworksExecution_compute(execution),
        nnapi_errno);
    if (res == ANEURALNETWORKS_NO_ERROR) { // by LDY
      uint64_t duration_hw;
      uint64_t duration_driver;
      nnapi_->ANeuralNetworksExecution_getDuration(execution,
	ANEURALNETWORKS_DURATION_ON_HARDWARE, &duration_hw);

      nnapi_->ANeuralNetworksExecution_getDuration(execution,
	ANEURALNETWORKS_DURATION_IN_DRIVER, &duration_driver);

      if (m_observer != NULL) {
        m_observer->update(duration_hw, duration_driver);
      }
    }
  }
...
StatefulNnApiDelegate::StatefulNnApiDelegate(Options options)
  ...
  m_observer = new ObserverDelegate(this); // by LDY
  m_hw_exe_time = -1; 			   // by LDY
  m_driver_exe_time = -1; 		   // by LDY
}
...
TfLiteStatus StatefulNnApiDelegate::DoPrepare(TfLiteContext* context,
  ...
  *nnapi_errno = 0;
  ...
  static const TfLiteRegistration nnapi_delegate_kernel = {
	...
	StatefulNnApiDelegate * delegate = static_cast<StatefulNnApiDelegate*>(params->delegate); // by LDY
	kernel_state->set_observer(delegate->get_observer()); // by LDY
      ...
      .invoke = [](TfLiteContext* context, TfLiteNode* node) -> TfLiteStatus {
        ...
        int* nnapi_errno =
            &(static_cast<Data*>(node->delegate->data_)->nnapi_errno);
        TfLiteStatus ret = state->Invoke(context, node, nnapi_errno);
	return ret;
      },
...
// by LDY
std::vector<std::string*> * StatefulNnApiDelegate::getDeviceNames() {
  const NnApi* nnapi = NnApiImplementation();
  uint32_t num_device_number = 0;
  std::vector<std::string * > * ret = NULL;
  if (nnapi->android_sdk_version >= kMinSdkVersionForNNAPI12) {
    ret = new std::vector<std::string *> (); 
    nnapi->ANeuralNetworks_getDeviceCount(&num_device_number);
    for (int i = 0; i < num_device_number; ++i) {
      ...
      ret->push_back(new std::string(dev_info));
    }
  }  
  return ret;
}
...
TfLiteDelegate* NnApiDelegate(StatefulNnApiDelegate::Options options) { // by LDY
  StatefulNnApiDelegate* delegate = new StatefulNnApiDelegate(options);
  return delegate;
}
