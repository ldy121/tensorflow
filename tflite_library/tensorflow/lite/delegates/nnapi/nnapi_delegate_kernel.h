...
class NNAPIDelegateKernel {
 const uint64_t init_time = 2000000000;
 public:
  NNAPIDelegateKernel() {
    nnapi_ = NnApiImplementation();
    m_burst =     NULL;		// by LDY
    m_observer =  NULL;		// by LDY
  }
  ~NNAPIDelegateKernel() {
    if (m_burst != NULL) { // by LDY
      nnapi_->ANeuralNetworksBurst_free(m_burst);
    }
    for (auto content : allocation_memory_mapping_) {
      nnapi_->ANeuralNetworksMemory_free(content.second);
    }
  }
  void set_observer(ObserverDelegate * observer) {
    m_observer = observer; // by LDY
  }
 ...
 private:
  ...
  TfLiteStatus BuildGraph(TfLiteContext* context,
                          ...
                          int* nnapi_errno);
  ANeuralNetworksBurst *m_burst; // by LDY
  ObserverDelegate *m_observer;   // by LDY
};
